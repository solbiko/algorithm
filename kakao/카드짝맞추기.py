...
def dfs(board, cdict, curr, order, cost): # 격자 카드, 좌표, 순열경우, 비용
    ...
    if choice1 < choice2:   # 적은 조작 횟수를 한 경우를 따라 재귀
        return dfs(new_board, cdict, cdict[idx][1], order[1:], cost + choice1)
    else:
        return dfs(new_board, cdict, cdict[idx][0], order[1:], cost + choice2)
